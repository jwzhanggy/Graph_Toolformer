...
> 
> **2.** If you plan to use the fine-tuned LLM checkpoint for graph reasoning, please (1) replace the fine-tuned checkpoints in the downloaded **kola/language_models/finetuned_checkpoint/** directory of the Graph_Toolformer_Package for graph reasoning demos, (2) also remember to change the checkpoint names (removing the datetime string at the end) before pasting to the koala folder, so the demo framework will load the checkpoint.
