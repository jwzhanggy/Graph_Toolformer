...
```
You can type in inputs which are similar to the prompt inputs, and the model will complete the reasoning and return the otuputs. The reasoning process will call both the LLMs and GNN models, so generating the output will take some time.
