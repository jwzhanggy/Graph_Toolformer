...
from code.Module_GPTJ_8bit import GPTJForCausalLM
...
class Method_Graph_Toolformer_GPTJ(method):
    ...
    def load_checkpoint(self, checkpoint_dir="./checkpoints/graph_toolformer"):
    ...
    def save_checkpoint(self, checkpoint_dir="./checkpoints/graph_toolformer"):
        date_str = datetime.today().strftime('%Y-%m-%d')
        torch.save({
        ...
        with torch.cuda.amp.autocast():
            for batch in test_dataloader:
                for index in range(len(batch['inputs'])):
                    ...
                    reasoning_output = batch['result'][index]
                ...
                count += 1
        return {'pred': pred_result, 'true': true_result, 'result': reason_result}
    ...
    def inference(self, input_text=''):
        ...
        sample_outputs = self.model.generate(
            ...
            num_return_sequences=5, max_length=self.max_length
        )
