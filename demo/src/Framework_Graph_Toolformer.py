...
class Framework_Graph_Toolformer:
    ...
    def prepare_setting(self):
        ...
        #self.language_model_hub.prepare()
    ...
    def api_calling(self, query):
        query_type = query[0][0]
        parameter_list = []
        for parameter in query[0][1]:
    ...
    def run(self):
        ...
        while True:
            ...
            query_dict = self.query_extraction(generation_output)
            result_dict = self.query_execution(query_dict)
            output_statement = self.statement_updating(generation_output, result_dict)
...
if __name__ == "__main__":
    ...
    if 0:
        ...
        generation_output = 'Output: The order of the dodecahedral graph is [GR(GL("gpr", {"dodecahedral_graph"}), "order")-->r]. Reasoning Result: 20.'
